...
Desktop App (Tauri) → GarageBuild Engine → SQLite + Local FS                    → AI Providers (local or cloud)

Self-hosted Server (Phase 5)
Web Studio (Browser) → REST API → GarageBuild Engine → SQLite / PostgreSQL                                  → AI Providers (local or cloud)

Air-gapped Enterprise (Phase 5)
Web Studio (Browser) → REST API (internal only) → GarageBuild Engine                                                  → Local AI only (Ollama / vLLM)

Monorepo Structure
garagebuild/├── packages/│   ├── engine/│   ├── server/      (Fastify REST API)│   ├── ui-desktop/  (Tauri + React)│   ├── ui-web/      (React SPA)│   ├── ui-vscode/   (VS Code extension)│   ├── cli/│   ├── plugin-sdk/│   └── plugins/│       ├── plugin-openai/│       ├── plugin-anthropic/│       ├── plugin-ollama/│       ├── plugin-react/│       └── plugin-docker/├── docs/├── examples/└── turbo.json

Implementation Status (Phase 1)
Subsystem / Package | Status
engine | ✅ All 10 subsystems implemented and tested
plugin-sdk | ✅ Full interface definitions
server | ✅ Fastify REST API, 40 tests
cli | ✅ Commander.js, status/model/create/cost commands
plugin-openai | ✅ GPT-4o, o1, streaming, cost tracking
plugin-react | ✅ Scaffold, templates, Dockerfile
plugin-docker | ✅ Build/run/stop, 24 tests
plugin-anthropic | ✅ Claude models, SSE streaming, cost tracking, 39 tests
plugin-ollama | ✅ Local models, dynamic discovery via /api/tags, zero cost, 26 tests
ui-web | ✅ React SPA, Dashboard/Projects/Models, 17 tests
ui-vscode | ✅ Extension, commands, status bar, 12 tests
ui-desktop | ✅ Tauri 2 + React, server management hook, 10 tests
...
Plugin Interfaces
Base
interface GarageBuildPlugin {  initialize(config: PluginConfig): Promise<void>;  teardown(): Promise<void>;  healthCheck(): Promise<HealthStatus>;  getManifest(): PluginManifest;  getConfigSchema(): ConfigSchema;}
...
Framework Plugin
interface FrameworkPlugin extends GarageBuildPlugin {  createProject(options: ProjectOptions): Promise<ProjectScaffold>;  generateComponent(spec: ComponentSpec): Promise<GeneratedFile[]>;  generatePage(spec: PageSpec): Promise<GeneratedFile[]>;  startDevServer(project: Project): Promise<DevServer>;  build(project: Project): Promise<BuildResult>;  generateDockerfile(project: Project): Promise<string>;}

Deployment Plugin
interface DeploymentPlugin extends GarageBuildPlugin {  deploy(project: Project, config: DeployConfig): Promise<DeployResult>;  generateDockerfile(project: Project): Promise<string>;  generateComposeFile(project: Project): Promise<string>;}

Agent Plugin
interface AgentPlugin extends GarageBuildPlugin {  execute(task: AgentTask): Promise<AgentResult>;  stream(task: AgentTask): AsyncIterator<AgentChunk>;  getCapabilities(): AgentCapability[];}
...
Plugin Distribution
Primary: npm  |  @garagebuild/plugin-name (official)  •  garagebuild-plugin-name (community)  •  @org/garagebuild-plugin-name (enterprise)
...
Architecture
GarageBuild Engine  →  ModelRouter  →  TokenCounter                             →  CostEngine                             →  StreamNormalizer                             →  PricingRegistry             →  OpenAI Plugin | Anthropic Plugin | Ollama Plugin
...
GarageBuild (व्योम)  •  Architecture Document v1.0  •  Build Once. Run Anywhere. Use Any AI. Own Everything.