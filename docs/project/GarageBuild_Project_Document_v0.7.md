...
1. Introduction
This document captures the current vision, discussions, decisions and future direction of the GarageBuild project.
...
About the Name
GarageBuild (व्योम) is a Sanskrit word meaning Infinite Space, Sky, Universe and Boundless. It reflects the core philosophy of the platform — no boundaries, no limits, no lock-in. Run anywhere. Use any AI. Own everything.
...
GarageBuild does not have one direct competitor. It overlaps several markets.
...
GarageBuild's unique position: The only open-source AI development platform that gives developers and enterprises a full local-first experience with complete model and infrastructure freedom.

8. Product Positioning
The platform is not another AI chatbot. Not another IDE. Not simply an AI app builder. GarageBuild is an AI-native software development environment.
...
10. Domain Model
The domain model defines the core concepts GarageBuild is built around. All platform decisions should map back to these entities.
...
Scope
Workspace
The root container for everything in GarageBuild
Global
Project
...
The plugin system is the core extensibility mechanism of GarageBuild. No modification to the GarageBuild core should ever be needed to add a new model, framework, deployment target or agent.

Golden Rule: If adding a new model, framework or deployment target requires modifying the GarageBuild core, the plugin architecture has failed.

Plugin Interfaces
Base Plugin Interface
interface GarageBuildPlugin {  initialize(config): Promise<void>;  teardown(): Promise<void>;  healthCheck(): Promise<HealthStatus>;  getManifest(): PluginManifest;  getConfigSchema(): ConfigSchema;}

Model Plugin Interface
interface ModelPlugin extends GarageBuildPlugin {  chat(request: ChatRequest): Promise<ChatResponse>;  stream(request: ChatRequest): AsyncIterator<ChatChunk>;  countTokens(text: string): Promise<number>;  estimateCost(inputTokens: number, outputTokens: number): CostEstimate;  getModelInfo(): ModelInfo;  listAvailableModels(): Promise<ModelDescriptor[]>;}

Framework Plugin Interface
interface FrameworkPlugin extends GarageBuildPlugin {  createProject(options): Promise<ProjectScaffold>;  generateComponent(spec): Promise<GeneratedFile[]>;  generatePage(spec): Promise<GeneratedFile[]>;  startDevServer(project): Promise<DevServer>;  build(project): Promise<BuildResult>;  generateDockerfile(project): Promise<string>;}

Deployment & Agent Interfaces
interface DeploymentPlugin extends GarageBuildPlugin {  deploy(project, config): Promise<DeployResult>;  generateDockerfile(project): Promise<string>;  generateComposeFile(project): Promise<string>;}interface AgentPlugin extends GarageBuildPlugin {  execute(task: AgentTask): Promise<AgentResult>;  stream(task): AsyncIterator<AgentChunk>;  getCapabilities(): AgentCapability[];}
...
Primary distribution: npm  |  Naming: @garagebuild/plugin-name (official)  •  garagebuild-plugin-name (community)
...
Deployment
Phase 1
@garagebuild/plugin-gemini
Model
Phase 2
@garagebuild/plugin-vue
Framework
Phase 4
@garagebuild/plugin-kubernetes
Deployment
Phase 5

12. Model Abstraction Layer
The bridge between the GarageBuild engine and every AI provider. The engine speaks one language and never knows which provider is underneath.
...
Monorepo Structure
garagebuild/├── packages/│   ├── engine/│   ├── ui-desktop/│   ├── ui-web/│   ├── ui-vscode/│   ├── cli/│   ├── plugin-sdk/│   └── plugins/│       ├── plugin-openai/│       ├── plugin-anthropic/│       ├── plugin-ollama/│       ├── plugin-react/│       └── plugin-docker/├── docs/├── examples/└── turbo.json

14. Product Requirements Document (PRD)
This PRD defines the requirements for the GarageBuild MVP — Phase 1. It is the specification a developer builds directly from.
...
Story
US-001
As a developer, I want to open GarageBuild and have a workspace ready immediately, so I can start building without any configuration.
US-002
...
US-012
As a developer, I want to export my project as a standard zip file, so I can take my code anywhere without GarageBuild.
...
US-021
As a developer, I want to ask GarageBuild to edit existing code by describing the change, so I don't have to manually find and modify files.
US-022
...
US-042
As a developer, I want a built-in terminal, so I can run commands without leaving GarageBuild.
...
Story
US-060
As a developer, I want GarageBuild to generate a Dockerfile for my project automatically, so I can containerize it without writing Docker config.
US-061
As a developer, I want GarageBuild to generate a docker-compose.yml, so I can run my app in a container locally with one command.
...
FR-002-05
System must never include GarageBuild-specific files in the exported project
...
NFR-014
Generated project code must run without GarageBuild installed
NFR-015
...
AC-001
A developer with no prior GarageBuild experience can install the desktop app and create their first AI-generated React component in under 5 minutes
AC-002
The generated component renders correctly in the live preview without any manual fixes
AC-003
The developer can ask GarageBuild to modify the component and the change appears in the preview within 2 seconds of the AI response completing
AC-004
The developer can export the project and run it with npm install && npm run dev without GarageBuild installed
AC-005
...
✅  Project name finalized: GarageBuild (व्योम)
...
GarageBuild (व्योम)  •  Version 0.7  •  Build Once. Run Anywhere. Use Any AI. Own Everything.