...
This document covers the engineering standards, conventions, processes and governance for the VYOM project. It is intended for contributors, maintainers and the core team.
...
Accepted
ADR-010
Local-first storage at ~/.vyom with SQLite database
Accepted

5. RFC Process
Significant changes to VYOM — new subsystems, changes to the plugin contract, new plugin types, breaking API changes — require an RFC (Request for Comments) before implementation.
...
Plugins in the VYOM ecosystem are categorised into four tiers. Certification determines discoverability and trust level in the marketplace.
...
1
Official
Built and maintained by the VYOM core team
Core team only
2
Verified
...
4
Experimental
Any npm package matching vyom-plugin-* naming
Automatic
...
Plugins declare minVyomVersion in their manifest
...
Fallback: AES-256 encrypted file at ~/.vyom/keys/
...
VYOM collects no telemetry by default. No usage data, no crash reports, no model usage statistics leave the user's machine without explicit opt-in.
...
Getting Started
Fork the repository at github.com/vyom-dev/vyom
Clone your fork: git clone https://github.com/your-username/vyom
...
Writing a Plugin
Install the plugin SDK: npm install @vyom/plugin-sdk
Implement the correct interface for your plugin type
Create a vyom-plugin.json manifest
...
Code of Conduct
VYOM follows the Contributor Covenant Code of Conduct. Be respectful, constructive and collaborative. Harassment, discrimination or personal attacks of any kind are not tolerated.

VYOM (व्योम)  •  Engineering Document v1.0  •  Build Once. Run Anywhere. Use Any AI. Own Everything.